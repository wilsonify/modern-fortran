module linear_algebra_mod
    ...
    implicit none
    private
    public :: invert_lower, premult_lower_by_transpose

    character(len=*), parameter :: modname = "linear_algebra_mod"
...
contains
    ...
    700 call err_handle(err, 300, called_from = subname // " in MOD " // modname)
        return
    710 call err_handle(err, 100, called_from = subname // " in MOD " // modname)
        return

    end function invert_lower

    integer(kind=our_int) function premult_lower_by_transpose(a, b, err) result(answer)
        implicit none
        real(kind=our_dble), intent(in) :: a(:, :)
        real(kind=our_dble), intent(out) :: b(:, :)
        type(error_type), intent(inout) :: err

        character(len = *), parameter :: subname = "premult_lower_by_transpose"
        integer(kind=our_int) :: p, i, j, k
    ...
    700 call err_handle(err, 300, called_from = subname // " in MOD " // modname)
        return
    710 call err_handle(err, 301, called_from = subname // " in MOD " // modname)
        return

...
contains
    ...
    pure logical function almost_equal(x, y, rtol, atol)
        ! almost_equal: element-wise fuzzy comparison for real/complex arrays
        real(r4), parameter :: default_rtol = 1.0e-5_real32
        real(r4), parameter :: default_atol = 1.0e-6_real32
        complex(real32), dimension(:), intent(in) :: x, y
        ...
        almost_equal = .true.
        do i = 1, size(x)
            if (.not. (abs(x(i) - y(i)) <= abs_tol + rel_tol * abs(y(i)))) then
                almost_equal = .false.
                exit
            end if
        end do
    ...
    pure real(r4) function xpowx_r4(x) result(res)
        real(r4), intent(in) :: x
        res = x**x
    end function xpowx_r4

    pure real(r8) function xpowx_r8(x) result(res)
        real(r8), intent(in) :: x
        res = x**x
...
program test_map
    ...
    tests(n) = assert(almost_equal(map(xpowx_r4, [1._real32, 2._real32, 3._real32]), &
